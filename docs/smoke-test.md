...
- **Edit status UI:** while an edit session is active, confirm the Remote edits panel lists the remote path and state. For a conflict/failed session, test Reveal, Re-download, Force upload after the confirmation prompt, and Close.
