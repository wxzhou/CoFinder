...
- **Remote HTML Open/Edit:** use remote `Open` or double-click on `.html` and `.htm` files and confirm the system default browser opens the app-managed local copy. Use `Edit` on the same files and confirm the configured text editor still opens them.
