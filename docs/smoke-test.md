...
- **Download fallback:** with a password-auth SFTP connection that can browse remotely but lacks passwordless rsync SSH, download a disposable remote file/folder and confirm the job succeeds over SFTP instead of staying failed with rsync exit code 255.
