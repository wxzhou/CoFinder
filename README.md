...
**`package.json` version:** `1.7.0`. **Latest completed development release:** **v1.7.0**.
...
- `v1.7.0` is the current development release target. Build artifacts use `release/CoFinder-1.7.0-arm64.dmg` and `release/CoFinder-1.7.0-arm64.zip` when `npm run dist` is executed.
