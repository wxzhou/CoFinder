...
Current development version: **v1.8.1** (`package.json` version `1.8.1`).
...
- rsync upload/download queue with conflict handling, retry, progress/status, and drag-and-drop between panes.
...
- Pane-scoped V12 UI with compact toolbars, breadcrumb copy buttons, resizable sidebar/panes, Inspector, transfer drawer, Preferences, diagnostics, and macOS menu integration.
...
- **Release versions** (`v1.8.1`, etc.) are semver versions used by `package.json`, tags, GitHub Releases, and build artifacts.
...
Build artifacts are generated under `release/`. For v1.8.1, expected artifact names are `CoFinder-1.8.1-arm64.dmg` and `CoFinder-1.8.1-arm64.zip`.
