...
export function V12VisualIconGrid<T extends FileEntry>(props: V12VisualIconGridProps<T>): ReactElement {
  const visibleGroups = props.groupByType
    ? groupEntriesByFileType(props.entries)
  ...
  return (
    ...
    >
      ...
      {visibleGroups.map((group) => (
        <section className="v12m-icon-type-group" key={group.id} aria-label={group.label || undefined}>
          {props.groupByType ? <div className="v12m-file-type-heading v12m-icon-type-heading">{group.label}</div> : null}
