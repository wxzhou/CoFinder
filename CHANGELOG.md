...
### M6 — full Rust migration, sidecar removed

- **Every IPC channel is now handled natively in Rust** (87/87; the only remaining entry in `channels.ts`, `system:resume`, has no handler and is dead config). The Node sidecar is **completely removed** from runtime and packaging: `tauri.conf.json` no longer declares `externalBin`, `cofinder_call` no longer falls back to a sidecar, and `scripts/build-sidecar-sea.mjs` is deleted. The app bundle dropped from **~101 MB to ~11 MB**.
- **M6a — simple system/local ops in Rust:** `local:openPath`/`revealPath` (via `open`/`open -R`), `local:compressGzip` (file→`.gz`, dir→`.tar.gz`), `system:quickLook` (`qlmanage -p`), `system:openTerminal`/`openSshTerminal`.
- **M6b — transfer queue in Rust:** full `TransferQueueService` port — enqueue upload/download/delete/gzip/decompress/md5/remoteCopy/remoteMove, `checkUploadConflicts`/`checkDownloadConflicts`, `list`/`cancel`/`stop`/`retry`/`retryFailed`/`clearCompleted`, lane concurrency (transfer/delete/compression/remoteMutation) + path locks, operation-lock dedup, `transfer:onUpdate` push, `failRemoteConnectionTasks`. Uploads/downloads run **rsync** (`ssh -p PORT -o BatchMode=yes`) with `--progress` parsing into percent/speed/eta/currentFile and fall back to a recursive **SFTP** transfer on ssh/rsync failures (exit 255, publickey/BatchMode, permission). All `RemoteService` instances share one global SFTP connection registry so the queue and the remote pane see the same live sessions.
- **M6c — remote ops in Rust:** chmod (`fsetstat`), touch, createTextFile, duplicate (≤50 MB), searchText (`rg`/`grep` over exec), readTextWindow (`awk` line window), compressGzip (exec gzip/tar).
- **M6d — remote edit/preview in Rust:** `remote:readPreview` (SFTP chunk read + magic-byte/UTF-8 sniff → text or base64 image), the full `RemoteEditService` port (session state machine, cache copy, polling local watcher with auto-sync, baseline conflict detection, syncNow/forceUpload/redownload/downloadConflictCopy/close, `remote:editUpdate` push), `content:openWindow` (opens the content webview and forwards `content:openRequest`), `remote:directorySizeStart/Cancel/Update`, `remote:previewOpen`/`previewClearFor*`, `remote:compressGzip`.
- Live read-only verification against sge (search/window reads) and EP9 (exec) as before; destructive and transfer write ops are not run against the live servers per project policy.
- `src/main` TypeScript sources and their vitest specs are kept as behavior reference (not packaged).

- **Rust backend progress:** the settings, local file service (browse/info/text reads/preview sniffing/touch/rename/delete/mkdir/create/search), profiles + Keychain credentials, local sidebar favorites, system channels (clipboard/logs/diagnostics/tool checks), and the SFTP remote core (connect/list/home/rename/delete/mkdir/readText/getInfo via `russh` + `russh-sftp`, **including public-key authentication**) are now handled natively in Rust. Remote connections via key auth (e.g. `~/.ssh/config` sge/EP9) are live-verified read-only. `cofinder_call` dispatches to Rust first and only falls back to the sidecar for channels not yet ported (transfers, remote edit/preview/window reads, chmod, duplicate, quick look), so behavior is unchanged throughout the transition. The app bundle is ~101 MB while the Node sidecar (~86 MB) still carries those not-yet-ported channels; it shrinks toward ~10 MB once they move to Rust and the sidecar is removed.
- **Fixed: secure-storage detection** — the Rust credential service now probes macOS secure storage with `security -h` (exit 0) instead of `security --help` (exit 2), so the "Save password" option is enabled and passwords persist to the Keychain again.
- **Fixed: private-key SFTP login** — the embedded connect and Site Manager forms previously rejected `privateKey` auth ("not yet supported") and forced `authType: password` on connect, so key-only servers (sge/EP9) could not log in. The forms now offer a private-key path field, validation requires it for key auth, and the connect request passes `authType` + `privateKeyPath` through to the Rust backend.
- The TypeScript main-process services still run in a bundled **Node sidecar** (built with esbuild + Node SEA) for the not-yet-ported channels; the renderer reaches services through Tauri `invoke`/`listen` via a `window.cofinder` bridge that preserves the `{ ok, data } | { ok, error }` IPC contract.
...
- Restored title-bar behavior in the Tauri shell: the title strip is draggable again and double-clicking empty title space (right of the tabs) toggles maximized state. Tauri's `data-tauri-drag-region` script blocks `dblclick`, so drag + double-click are driven manually via `startDragging()` / `toggleMaximize()` with tabs/buttons excluded.
