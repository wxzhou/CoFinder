...
- Remote `Change Permissions` now uses an in-app octal-mode dialog instead of relying on `window.prompt`, so the context-menu action gives visible feedback and validation.
